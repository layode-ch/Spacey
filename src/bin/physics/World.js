...
export class World extends Array {
    ...
    #activeCollisions;
    #deltaTime;
    constructor(gravity = new Vector(0, 2), ...colliders) {
        super(...colliders);
    ...
    #resolveCircleRectCollision(circleCollider, rectCollider) {
        const circle = circleCollider.shape;
        const rect = rectCollider.shape;
        // Step 1: Find closest point on rectangle to circle center
        const closestX = Math.max(rectCollider.x, Math.min(circleCollider.x, rectCollider.x + rect.width));
        const closestY = Math.max(rectCollider.y, Math.min(circleCollider.y, rectCollider.y + rect.height));
