...
import { CollideEventArgs, Collider, Vector, World } from "./physics/index.js";
import { Rectangle } from "./shapes/Rectangle.js";
import { Spaceship, Alien, EntityManager, Bullet, Entity } from "./entities/index.js";
...
export class Game {
	...
	#walls = new Map<string, Collider>();
	#bulletSample: AudioBuffer;
	...
	#playSample(sample: AudioBuffer, rate: number) {
	...
	#random(min: number, max: number, precision: number = 1): number {
		return Math.floor(((Math.random() * (max - min)) + min) * precision) / precision;
	}

	#alienOncollide(sender: Collider, args: CollideEventArgs) {
	...
	#alienOnhpchanged(sender: Entity) {
		const alien = sender as Alien;
		if (sender.hp <= 0) {
			this.score += 1;
	...
	#bulletOncollide(sender: Collider, args: CollideEventArgs) {
